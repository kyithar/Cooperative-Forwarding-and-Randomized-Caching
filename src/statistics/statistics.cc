...
void statistics::finish(){
    ...
    uint32_t global_unwanted_data= 0;
    uint32_t global_requests= 0;
    ...
    for (int i = 0; i<num_nodes; i++)
    {
        ...
        if (cores[i]->interests){
            ...
            global_unwanted_data += cores[i]->unwanted_data;
    ...
    sprintf (name, "interest_check_by_CS");
    recordScalar(name,(global_hit+global_miss)/global_interests);
    cout<<"interest_check_by_CS: "<<(global_hit+global_miss)/global_interests<<endl;

    sprintf ( name, "interests");
    recordScalar(name,global_interests * 1./num_nodes);

    sprintf ( name, "total_interests");
    recordScalar(name,global_interests);

    sprintf ( name, "data" );
    recordScalar(name,global_data * 1./num_nodes);

    sprintf ( name, "total_data");
     recordScalar(name,global_data);

    sprintf ( name, "unwanted_data" );
        recordScalar(name,global_unwanted_data * 1./num_nodes);
        cout<<"unwanted_data/nodes: "<<global_unwanted_data * 1./num_nodes<<endl;
    ...
    for (int i = 0;i<num_clients;i++){
        ...
        global_requests  += clients[i]->get_tot_downloads();
        //<aa>
        #ifdef SEVERE_DEBUG
    ...
    sprintf ( name, "total_requests");
    recordScalar(name,global_requests);
    cout<<"Time/client: "<<global_requests<<endl;

    sprintf ( name, "Interest traffic");
    recordScalar(name,(double) ((global_hit+global_miss)-global_requests)/(global_hit+global_miss));
    cout<<"interest traffic: "<<((global_hit+global_miss)-global_requests)/(global_hit+global_miss)<<endl;
