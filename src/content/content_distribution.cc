...
for(int i=0; i<t_router; i++){
    if(clid[i]!=-1){
       CLUCOUNT[clid[i]].total_c += cacheini[i];
                  }

}
cout<<"CLU count test "<<CLUCOUNT[0].total_c<<endl;
cout<<"CLU count test "<<CLUCOUNT[1].total_c<<endl;

for(int i=0; i<t_router; i++){

if(clid[i]!=-1){
    cache_size = cacheini[i];//cache size of current router
    total_cache_size = CLUCOUNT[clid[i]].total_c;//total cache size for each cluster
        key_dis = round(num_of_vr*(cache_size/total_cache_size));
        CHW[i].id=key_dis ;
        cout<<i<<" router's key dis "<<CHW[i].id<<endl;
}
...
cout<<"size of CLcount "<< CLUCOUNT.size()<<endl;

for(int i=0; i<t_router; i++){

    if(clid[i]==0){
            for (int d = 0; d < CHW[i].id; d++){

                                add_range =myvector[basec1];
                                CH0[add_range]=i;
                                basec1++;
                                //if(basec1==num_of_vr){basec1=0;}
                               cout<<"cluster "<<clid[i]<<"-Router- "<<i<<"-is added keys-"<<add_range<<" to"<<CH0[add_range]<<endl;
                            }
            cout<<"*************************"<<endl;
        }
...
for(int i=0; i<t_router; i++){
    ...
    if(clid[i]==1){
            for (int d = 0; d < CHW[i].id; d++){
                                ...
                                basec1++;
                                //if(basec1==num_of_vr){basec1=0;}
                               cout<<"cluster "<<clid[i]<<"-Router- "<<i<<"-is added keys-"<<add_range<<" to"<<CH1[add_range]<<endl;
                            }
            cout<<"*************************"<<endl;
        }
...
//                      total_cache_size = CLUCOUNT[clid[i->first]].total_c;//total cache size for each cluster
...
//void content_distribution::populate_CHT(){
//    int add_range;
//    int basec1=0;
//    vector<int> myvector;
////%%%%%%%%%%%%%%%  to generate key for CH (Start)
...
//                                             basec1++;
//                                             if(basec1==num_of_vr){basec1=0;}
//                                            cout<<"cluster "<<clid[i->first]<<"-Router- "<<i->first<<"-is added keys-"<<add_range<<" to"<<CH0[add_range]<<endl;
//                                         }
//                         cout<<"*************************"<<endl;
//                     }
//
...
//                                                          if(basec1==num_of_vr){basec1=0;}
//
//                                                          cout<<"cluster "<<clid[i->first]<<"-Router- "<<i->first<<"-is added keys-"<<add_range<<" to"<<CH1[add_range]<<endl;
//                                                      }
//                                      cout<<"*************************"<<endl;
//                                  }
//
