...
arrival +=1;
        if(int_msg->getCount()!=1){
            interests++;
        }

        if(bridge[getIndex()]==1){
            if(clid[getIndex()]!=int_msg->getClid()){
                int_msg->setI_type(0);
                int_msg->setCount(0);
                int_msg->setClid(clid[getIndex()]);
               // cout<<getIndex()<<" bridge reset the interest type to zero"<<endl;
            }
...
//
//
		if(clid[getIndex()]!=-1){//search the custodian router
		            int custo_r=CHring[int_msg->getChunk() % num_of_vr];
		            if(current_r==custo_r){//should stop at custo
		                if(int_msg->getCount()==1){
		                    if(ContentStore->fake_lookup(int_msg->getChunk())==true){
	                            Cstat[int_msg->getChunk()].req_count +=1;
	                            cout<<"chunk counter "<<Cstat[int_msg->getChunk()].req_count<<endl;
		                    }
...
//    if(ContentStore->get_tot_req()<arrival){
        w = arrival;
//    }else{
//        w =ContentStore->get_tot_req();
//    }
...
void core_layer::handle_cdecision(ccn_interest *int_msg){
    chunk_t chunk = int_msg->getChunk();
    if(Cstat[chunk].cache_cost==-1){
        if(w==0){
             Cstat[chunk].cache_cost=10;
                 }else{
                 Cstat[chunk].cache_cost=w;
                }
        Cstat[chunk].q=pow(1+(1/Cstat[chunk].cache_cost),Cstat[chunk].cache_cost)-1;
    }



          double p_old=Cstat[chunk].p;
          Cstat[chunk].p=(p_old*(1+(1/Cstat[chunk].cache_cost)))+(1/(Cstat[chunk].q*Cstat[chunk].cache_cost));

           double tmp_ran=uniform(0, 1);
           if(Cstat[chunk].p > tmp_ran){
                     Cstat[chunk].CB=1;
                 }

                 if(Cstat[chunk].p>1){
                     cout<<"error"<<Cstat[chunk].p<<endl;
                 }
...

{
	...
	double int_btw = int_msg->getBtw();
	    if(clid[getIndex()]!=-1){//search the custodian router
	        custo_r=CHring[chunk % num_of_vr];
	        int_msg->setCusto(custo_r);
	    }else{//if the current node is base station set -1 as a custodian name
	        int_msg->setCusto(-1);
	    }


	    if(current_r==0||current_r==1){//current routers are content servers
    ...
    }else{//current routers are normal routers

        if ((custo_r==current_r||clid[current_r]==-1) && ContentStore->lookup(chunk)){
            Cstat[chunk].custodian_hit +=1;
           //
           //a) Check in your Content Store
           //
           ccn_data* data_msg = compose_data(chunk);
        ...
        unordered_map < chunk_t , pit_entry >::iterator piIt = PIT.find(chunk);

        //<aa>
        //</aa>

        if(clid[current_r]!=-1){//cluster
            if(custo_r == current_r){//current router is custodian
                if(piIt!=PIT.end()){// request is already forwarded
                    int_msg->setI_type(0);
                }else{// request is not forwarded yet
                    int_msg->setI_type(1);
                }

            }else{//current router is non custodian
                if(piIt!=PIT.end()){// request is already forwarded
       ...
       // }
        ...
        if(custo_r == current_r||clid[current_r]==-1){// BS and Custodian


            handle_cdecision(int_msg);
            }else{//non custodian
            if(piIt!=PIT.end()){
                if(PIT[chunk].custo_int==1){
                    int_msg->setCount(1);
                }
            }
		...
		//</aa>
//if(pitIt==PIT.end()){
//    cout<<getIndex()<<" PIT is clean"<<endl;
//}else{
//    cout<<getIndex()<<" PIT is not clean"<<endl;
//}
//if(pitIt != PIT.end() && int_msg->getNfound() ){
//    cout<<getIndex()<<" pitIt != PIT.end() && int_msg->getNfound()"<<endl;
//}
//if(simTime() - PIT[chunk].time > 2*RTT){
//    cout<<getIndex()<<" simTime() - PIT[chunk].time > 2*RTT"<<endl;
//}
//if(int_msg->getI_type()==1){
//    cout<<getIndex()<<" int_msg->getI_type()==1"<<endl;
//}
//if(int_msg->getCount()==1){
//    cout<<getIndex()<<" int_msg->getCount()==1"<<endl;
//}

int_msg->setClid(clid[current_r]);
        ...
        if (
			...
			//the custodian router update the interest to prevent the interest blocking by the intermediate routers
			||int_msg->getI_type()==1
			||int_msg->getCount()==1
...
void core_layer::handle_data(ccn_data *data_msg)
{
    ...
    unordered_map < chunk_t , pit_entry >::iterator pitIt = PIT.find(chunk);
    //cout<<Cstat[chunk].custodian_hit<<endl;
	//<aa>
	#ifdef SEVERE_DEBUG
		int copies_sent = 0;
	#endif
    ...
    if ( pitIt != PIT.end() )
	{
        if(Cstat[chunk].CB==1){
                 ...
                 //if(Cstat[chunk].req_count>10){
                   data_msg->setCur_req(Cstat[chunk].req_count);
                     ContentStore->store(data_msg);
                    Cstat.erase(chunk);
                 //}
...
//
//
//       double request_cost,invest_cost,cached_decision,min_cost,received_money,revenue,cache_space_cost;
//
//        chunk_t chunk = data_msg -> getChunk(); //Get information about the file
//
//        received_money=1;// selling price
//        request_cost = 1;// request cost from current router to neighbor router
//        cache_space_cost =30;// cache space cost for 1 chunk
//        invest_cost = cache_space_cost +(request_cost* Cstat[chunk].cache_miss);
//        revenue = received_money* Cstat[chunk].access_freq;
//        if(revenue < invest_cost){min_cost= revenue;}
//        else{min_cost=invest_cost;}
//        cached_decision=round((revenue+invest_cost)/min_cost);
////cout<<"cached_decision->"<<cached_decision<<"-miss->"<<Cstat[chunk].cache_miss<<endl;
