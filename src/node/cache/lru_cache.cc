...
void lru_cache::data_store(ccn_data *data_msg){
    ...
    p->newer = 0;
    p->older = 0;
    p->tot_req=data_msg->getCur_req();
    //cout<<"lru req update "<<data_msg->getCur_req()<<endl;
...
double lru_cache::get_req(){
    double temp_sum=0;
    double temp_count = 0;
    for (unordered_map<chunk_t, lru_pos*>::iterator i = cache.begin();i != cache.end();i++){
            temp_sum +=cache[i->first]->tot_req;
            temp_count++;
