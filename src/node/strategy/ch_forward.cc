...
Register_Class(ch_forward);



bool *ch_forward::get_decision(cMessage *in){
...
bool *ch_forward::exploit(ccn_interest *interest){
...
if(custo_r==-1){//BS
    ...
    if(custo_r!=getParentModule()->getIndex() && interest->getI_type()!=1 ){//non custo
    const int_f FIB_entry = get_FIB_entry(custo_r);
    outif = FIB_entry.id;
    bool *decision = new bool[gsize];
    std::fill(decision,decision+gsize,0);
...
int ch_forward::nearest(vector<int>& repositories){
